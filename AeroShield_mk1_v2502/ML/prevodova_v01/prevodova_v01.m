...
sstime_steps = (0 : length(u_prevod_vals)) * sstime_width;
...
while true
    ...
    if (time_delta >= T_sample * 1000)
        ...
        fprintf(datafileID, '%8.3f, %6d, %8.3f, %8.3f, %8.3f, %8.3f\n', tmp_printlist);
...
%         ylim([-127, 1023]);
        ...
        tmpidx = find(sstime_steps <= time_elapsed, 1, 'last');

        if tmpidx <= length(u_prevod_vals);
            u = u_prevod_vals(tmpidx);
        end
...
copyfile("./dataRepo/" + "dataFile_" + DateString + ".txt", "./dataRepo/" + "dataFile_" + "_last.txt");
